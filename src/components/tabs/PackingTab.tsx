...
export function PackingTab({ tripId }: PackingTabProps) {
  ...
  const handleTogglePacked = (itemId: Id<"packingItems">) => {
    togglePacked({ itemId }).catch(() => {
      toast.error("Failed to update item");
    });
  };

  const handleDelete = (itemId: Id<"packingItems">) => {
    removeItem({ itemId })
      .then(() => {
    ...
    <div className="space-y-6">
      ...
      {categories.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            Start building your packing checklist!
          </p>
        </div>
      ) : (
              ...
              <div className="space-y-2">
                {groupedItems[category].map((item) => (
                  ...
                  >
                    ...
                    />
                    <span
                      className={`flex-1 ${item.packed ? "line-through text-gray-500" : "text-gray-900"}`}
                    >
                      {item.item}
                    </span>
                    <button
