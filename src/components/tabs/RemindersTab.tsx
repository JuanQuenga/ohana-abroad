...
export function RemindersTab({ tripId }: RemindersTabProps) {
  ...
  const handleToggleCompleted = (reminderId: Id<"reminders">) => {
    toggleCompleted({ reminderId }).catch(() => {
    ...
    <div className="space-y-6">
      ...
      {reminders.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            Add reminders for important tasks and deadlines!
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {sortedReminders.map((reminder) => {
            ...
            const isDueSoon =
              daysUntil <= 3 && daysUntil >= 0 && !reminder.completed;
